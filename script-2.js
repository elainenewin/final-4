...
// ─── CUBE STATE ──────────────────────────────────────────────
function setCubeState(cubeClass, labelText, labelClass) {
  rubikCube.classList.remove('cube-holding', 'cube-running', 'cube-pb');
  cubeLabel.classList.remove('label-holding', 'label-running', 'label-pb');
  if (cubeClass) rubikCube.classList.add(cubeClass);
  cubeLabel.textContent = labelText;
  if (labelClass) cubeLabel.classList.add(labelClass);
}
...
function setPhase(phase) {
  ...
  if (phase === 'idle') {
    ...
    soundReady();
    setCubeState('cube-holding', 'Ready!', 'label-holding');
    ...
    soundStart();
    state.rafId = requestAnimationFrame(tick);
    setCubeState('cube-running', 'Solving!', 'label-running');
...
function onNewPB() {
  ...
  setCubeState('cube-pb', '🏆 PB!', 'label-pb');
  setTimeout(() => setCubeState(null, 'Done ✓', null), 2000);
  soundPB();
  launchConfetti();
}
